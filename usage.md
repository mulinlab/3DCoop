...
Two datasets for testing, one for 3D mode and one for 1D mode, has been included in the repo. They can be found in the `test` folder. Besides, the configuration file and pipeline running script are provided in the `test` folder. They can be referred by users.

The datasets for K562 cell line which we used as the example in the *STAR Protocols* are also provided in the `K562` folder. They can be used to reproduce the whole process according to our protocol.

The details for inputs, configuration file, and pipeline running steps will be described in this manual.

## Inputs

Two types of input datasets (`interactions` and `peaks` for 3D mode while only `peaks` for 1D mode) are needed for 3DCoop, so we organize them into one folder named `inputs`:
...
The peaks or TR binding events are organized for each tissue/cell type. Please make a folder for each tissue or cell type, and put all files of peaks in this folder. Here we call the tissue/cell type as `CELLID` for short.
...
This file stores the genomic intervals from chromatin loops. The `BED` file can be got from `BEDPE` file using the `bin/bedpe2bed.pl` script. A file named `CELLID.bed` will be generated besides `CELLID.bedpe`, such as `K562.bed` besides `K562.bedpe`.
...
A configuration file to tune the whole pipeline is needed. This configuration file can be renamed as users want, and the given name will be passed as a parameter to the pipeline scripts . Here the name `3DCoop_test.cfg` is used for example.
...
* `dir_in`: folder for inputs. It should include peaks and interactions for 3D mode, and only peaks or 1D mode.
...
* `fraction`: a minimal overlap fraction of peak to assign it to a certain interaction. Multiple values can be specified by separating using `,`. *Make sure* the length is same to the length of `cells`.
...
Here, the `DIR_TO_BIN` is the path to the `bin` folder of the pipeline. The `FILE_OF_CFG` is the path to the configuration file, such as `3DCoop_test.cfg`.

## Outputs

Six folders corresponding to each step will be generated in the output folder which is specified in the configuration file.

* `01_intersection_bed`: corresponding to STEP1, the results in `BED` format by intersecting peaks and chromatin interactions.
* `02_intersection_bedpe`: corresponding to STEP1, the results in `BEDPE` format by intersecting peaks and chromatin interactions. This folder is not avaliable for 1D mode.
* `03_jaccard`: corresponding to STEP2, the Jaccard results (TR pair-wise correlation matrix based on the generalized Jaccard similarity) for TR pairs.
* `04_glasso`: corresponding to STEP3, the Glasso results (precision matrix) for TR cooperation.
* `05_clusterone`: corresponding to STEP4, the results from ClusterONE and modularity analysis of TR cooperation. There are several folders in this folder, while the folder named `08_results` is the final results that users will need.
* `06_network`: corresponding to STEP5, TR cooperation networks in `PDF` and `PNG` format.
...
* `CELLID_pairs.txt`: TR pairs extracted from clusters and related scores including Jaccard and Glasso, with the first line as the header.
...
* The outputs will be saved in a sub-folder named `07_variants2TRs` in the main output folder.
...
Nearly all databases have collected TR motifs for many species. Here we use the data for human (*Homo Sapiens*) as the example. The process for data of other species is similar.
