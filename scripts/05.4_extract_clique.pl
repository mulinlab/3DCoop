...
if ( $conf{"extract_clique"} =~ /yes/i ) {
    ...
    my $dout_clique = "$din_co/04_cliques";
    mkdir $dout_clique unless -d $dout_clique;

    my $r = "$FindBin::Bin/05.4.0_cluster2clique.R";
    ...
    foreach my $cell (@cells) {
        ...
        while (<$IC>) {
            ...
            for ( my $i = 0 ; $i < $#tfs ; $i++ ) {
                ...
                for ( my $j = $i + 1 ; $j <= $#tfs ; $j++ ) {
                    ...
                    if ( exists $jaccard{$key} && exists $glasso{$key} ) {
        ...
        select $O;
        print
          "clique\tnumber\tjaccardSum\tjaccardMean\tglassoSum\tglassoMean\n";
